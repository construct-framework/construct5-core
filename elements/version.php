...
class JFormFieldVersion extends JFormField
{
	...
	protected function getLabel()
	{
		...
		if (!extension_loaded('curl')) {
			return JText::_('TPL_CONSTRUCT5_CORE_ERROR_NOCURL');
		} // If cURL is supported, check the current version available.
		else {
			// Get the module's XML
			$xmlfile = JPATH_SITE . '/templates/construct5-core/templateDetails.xml';
			...
			$message = '<label style="max-width:100%">' . JText::sprintf('TPL_CONSTRUCT5_CORE_VERSION_INSTALLED', $name, $version);
			...
			if (version_compare($version, $update,'lt')) {
				$message .= '  <a href="http://construct-framework.com" target="_blank">' . JText::sprintf('TPL_CONSTRUCT5_CORE_VERSION_UPDATE', $update) . '</a></label>';
			} // No updates, or the Joomla! version is not compatible, so let the user know they're using the current version
			else {
				$message .= '  ' . JText::_('TPL_CONSTRUCT5_CORE_VERSION_CURRENT') . '</label>';
			}
			return $message;
		}
	}
